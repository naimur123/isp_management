...
$sql = 'SELECT cr.*, c.customer_id AS cust_code, c.customer_name, 
        ...
        ORDER BY cr.billing_month DESC, c.customer_name ASC';

$collection_records = run_all($sql , $collectionParams);

/* Get Top 10 collection records */
$sqlTop10 = 'SELECT c.id AS customer_id_pk, c.customer_id AS cust_code, c.customer_name, 
                    co.company_name, cat.category_name, z.zone_name,
                    SUM(cr.billing_amount) AS total_paid,
                    COUNT(cr.id) AS total_months
             FROM collection_records cr
             JOIN customers c ON c.id = cr.customer_id
             JOIN companies co ON co.id = c.company_id
             JOIN categories cat ON cat.id = c.category_id
             ...
             GROUP BY c.id, c.customer_id, c.customer_name, co.company_name, cat.category_name, z.zone_name
             ORDER BY total_paid DESC
             LIMIT 10';
$top10_collection_records = run_all($sqlTop10, $topParams);
/* End */
...
<div class="row g-3 mb-1">
  <div class="col-12">
    <div class="fw-bold small text-uppercase text-muted">Month to Month Collection Highlights</div>
  </div>
  <!-- Total Collection -->
  <div class="col-lg-4">
    <div class="card h-100">
      <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <a href="<?= e(base_url('billing/collection_list.php')) ?>?<?= http_build_query($_GET) ?>" 
      ...
      <div class="table-responsive">
        <table class="table table-m">
          <tbody>
            <?php if (!$collection_records): ?><tr>
                <td class="text-muted small p-3">No data.</td>
              </tr><?php endif; ?>
            <?php foreach ($collection_records as $i => $r): ?>
              <tr>
                <td><?= $i + 1 ?></td>
                <td><?= e(format_month($r['billing_month'])) ?><br><span class="text-muted small"><?= e($r['customer_name']) ?></span><br><span class="text-muted small"><?= e($r['company_name']) ?></span>
                </td>
                <td class="text-end fw-semibold"><?= format_currency($r['billing_amount']) ?><br><span class="text-muted small"><?= status_badge($r['status']) ?></span>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
  ...
  <div class="col-lg-4">
    <div class="card h-100">
      ...
      <div class="table-responsive">
        <table class="table table-m">
          <tbody>
            <?php if (!$top10_collection_records): ?><tr>
                <td class="text-muted small p-3">No data.</td>
              </tr><?php endif; ?>
            <?php foreach ($top10_collection_records as $i => $r): ?>
              <tr>
                <td><?= $i + 1 ?></td>
                <td><?= e($r['customer_name']) ?><br><span class="text-muted small"><?= e($r['company_name']) ?></span>
                </td>
                <td class="text-end fw-semibold"><?= format_currency($r['total_paid']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
